...
interface RepoEntry {
  id:         string;
  dbRepoId?:  string;   // path-hash ID stored in PostgreSQL (set after first analysis)
  path:       string;
  label:      string;
  color:      string;
  status:     RepoStatus;
...
function makeRepo(path: string): RepoEntry {
  ...
  return { id: crypto.randomUUID(), dbRepoId: undefined, path, label, color: nextColor(), status: 'idle', progress: 0, totalFiles: 0, log: [], nodes: [], edges: [], summary: null };
}
...
function decodeToken(t: string): any {
  try {
...
export default function App() {
  ...
  const handleLoginSuccess = useCallback((t: string, u: string) => {
  ...
  // Parse URL hash for token on mount/load
  useEffect(() => {
    ...
    if (hash && (hash.startsWith('#token=') || hash.includes('token='))) {
      ...
      if (t) {
        ...
        if (u) {
          ...
          const name = decoded?.username || 'Developer';
  ...
  // Countdown timer removed, session now explicitly managed via logout

  // Load previously analyzed repos from database when user is authenticated
  useEffect(() => {
    if (!token) {
      setRepos([]);
      return;
    ...
    const fetchRepos = async () => {
      try {
        ...
        const res = await fetch(`${API}/api/repos`, { headers });
        if (res.ok) {
          const json = await res.json();
          if (json.success && Array.isArray(json.repositories)) {
            // Map colors to each loaded repository
            const loadedRepos = json.repositories.map((r: any) => ({
              ...r,
        ...
        console.error('[fetchRepos] Failed to load repositories:', err);
      }
    };
    fetchRepos();
  }, [token]);
  ...
  const removeRepo = (id: string) => {
    ...
    if (r) {
      ...
      // Use the DB repo ID stored when analysis completed, fallback to graph node data
      const actualRepoId = r.dbRepoId ?? r.nodes?.[0]?.data?.repoId;

      if (actualRepoId) {
        console.log('[removeRepo] Deleting repo from DB with id:', actualRepoId);
        const headers: Record<string, string> = { 'Content-Type': 'application/json' };
        if (token) headers['Authorization'] = `Bearer ${token}`;
        fetch(`${API}/api/repo`, {
          method: 'DELETE',
          headers,
          body: JSON.stringify({ repoId: actualRepoId }),
        })
          .then(res => res.json())
          .then(json => console.log('[removeRepo] Delete response:', json))
          .catch(err => console.error('[removeRepo] Failed to delete repo from backend:', err));
      } else {
        console.warn('[removeRepo] No dbRepoId found — repo may not have been analyzed yet, skipping DB delete.');
      }
    }
    setRepos(rs => rs.filter(x => x.id !== id));
  };
        ...
        for (const part of buf.split('\n\n').slice(0, -1)) {
          ...
          try {
            ...
            setRepos(rs => rs.map(r => {
              ...
              if (evt.total != null)    nx.totalFiles = evt.total;
              if (evt.phase === 'result') {
                ...
                if (evt.resolvedRepoId)  nx.dbRepoId = evt.resolvedRepoId;
  ...
  const runQuery = async () => {
    ...
    setQuerying(true); setQueryResult(null); setQueryFiles([]); setQueryError(null); setLastSynthesisError(null);
    try {
      const payload: any = { query: query.trim() };
      if (apiKey.trim()) payload.apiKey = apiKey.trim();
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res  = await fetch(`${API}/api/query`, { method: 'POST', headers, body: JSON.stringify(payload) });
      const json = await res.json();
      if (res.ok && json.success) {
        setQueryResult(json.analysis); 
        setQueryFiles(json.relevantFiles ?? []);
        setLastSynthesisError(json.synthesisError || null);
      } else {
        setQueryError(json.error || `Error ${res.status}`);
      }
    } catch (e: any) {
      setQueryError(e?.message || 'Network error: Backend unreachable');
    }
    ...
    <div className="app-shell">
      ...
      <nav className="app-nav">
        ...
        <div className="app-nav-right">
          {timeLeft !== null && (
            <div className="status-pill ok" style={{ background: 'rgba(34, 197, 94, 0.1)', borderColor: 'rgba(34, 197, 94, 0.3)', color: '#22c55e', fontWeight: 600 }}>
              ⏳ {formatTime(timeLeft)}
            </div>
          )}
          {username && (
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--gray-600)', marginLeft: 8 }}>
              👤 {username}
            </div>
          )}
          <button className="btn btn-outline btn-sm" onClick={handleLogout} style={{ padding: '4px 8px', fontSize: 11, marginLeft: 8 }}>
            Logout
          </button>
            ...
            <div className="side-scroll">
              ...
              {activeTab === 'query' && (
                <div className="fade-up">
                  ...
                  <div className="side-section">
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Gemini API Key (Optional)</div>
                    <input className="input" type="password" style={{ width: '100%', marginBottom: 16, fontSize: 13.5 }} value={apiKey} 
                      onChange={e => setApiKey(e.target.value)} 
                      placeholder="Overrides server .env key if provided..." />
                  ...
                  {queryFiles.length > 0 && !queryResult && !querying && (
                    <div className="side-section" style={{ background: '#fffbeb', borderLeft: '3px solid #f59e0b' }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#92400e', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>ΓÜá AI Synthesis Warning (V2)</div>
                      <p style={{ fontSize: 12.5, color: '#78350f', lineHeight: 1.6 }}>
                        Local search found relevant files, but the AI was unable to generate a text explanation. 
                        {lastSynthesisError && <div style={{ marginTop: 8, padding: '8px', background: 'rgba(255,255,255,0.5)', borderRadius: 4, fontFamily: 'var(--font-mono)', fontSize: 11, color: '#92400e' }}><strong>Error Details:</strong> {lastSynthesisError}</div>}
                        {!lastSynthesisError && <div style={{ marginTop: 6 }}>No specific error details provided. This usually means the <strong>GEMINI_API_KEY</strong> is missing or invalid in the backend .env file.</div>}
                      </p>
                    </div>
                  )}
                  ...
                  {queryResult && (
                    <>
                      <div className="side-section" style={{ background: '#f0fdf4', borderLeft: '3px solid #22c55e' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#15803d', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>✦ AI Explanation</div>
