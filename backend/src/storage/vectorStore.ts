...
// ─── In-memory store (loaded from PostgreSQL on startup / after ingestion) ─────
// Vector search is still performed in-memory for speed; PostgreSQL is the
...
/** Persist the current in-memory vector store to PostgreSQL via the Java backend. */
export async function persistVectorStore(): Promise<void> {
    await getStore().saveVectors(_docs);
}
...
export async function addRichDocument(doc: Omit<RichVectorDoc, 'vector'>, _apiKey?: string) {
    ...
    try {
        const key = _apiKey || process.env.GEMINI_API_KEY;
        const embedding = key
            ? await geminiEmbed(doc.compositeText, key)
            : await ollamaEmbed(doc.compositeText);
 ...
 *   2. Re-embed the new compositeText using Ollama/Gemini.
...
export async function upsertDocument(doc: Omit<RichVectorDoc, 'vector'>, _apiKey?: string): Promise<'updated' | 'inserted' | 'error'> {
    // Remove stale entry from in-memory store
    const prevIdx = _docs.findIndex(d => d.filePath === doc.filePath);
    if (prevIdx !== -1) _docs.splice(prevIdx, 1);

    let fullDoc: RichVectorDoc;
    try {
        const key = _apiKey || process.env.GEMINI_API_KEY;
        const embedding = key
            ? await geminiEmbed(doc.compositeText, key)
            : await ollamaEmbed(doc.compositeText);
        fullDoc = { ...doc, vector: embedding };
        _docs.push(fullDoc);
    } catch (err: any) {
    ...
    // Persist the single updated document to PostgreSQL immediately
    try {
...
export async function semanticSearch(
    ...
    // Attempt vector search; fall back to keyword search if embed fails
    let results: RichVectorDoc[];
    try {
        const key = apiKey || process.env.GEMINI_API_KEY;
        const queryVec = key
            ? await geminiEmbed(query.trim(), key)
            : await ollamaEmbed(query.trim());
