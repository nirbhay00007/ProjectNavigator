...
export async function runIngestionPipeline(
    ...
    const frontendRepoId = options.repoId ?? 'default';
    let repoId           = frontendRepoId;
    const repoLabel = options.repoLabel ?? (options.targetPath ? path.basename(options.targetPath) : 'repo');

    if (_runningRepos.has(frontendRepoId)) throw new Error(`Repo ${frontendRepoId} is already being ingested.`);
    _runningRepos.add(frontendRepoId);
    ...
    try {
        ...
        const store = initStore(targetPath, options.token);
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // PHASE 2+3 — Concurrent Ollama/Gemini summarization + vector store embedding
        // ═══════════════════════════════════════════════════════════════════════
        const activeKey = options.apiKey || process.env.GEMINI_API_KEY;
        const concurrencyLimit = activeKey ? 8 : CONCURRENCY;
        const limit = createSemaphore(concurrencyLimit);
        let done = 0;
        ...
        await Promise.all(rawNodes.map(node => limit(async () => {
            ...
            try {
                ...
                if (cached) {
                    ...
                    if (activeKey) {
                        try {
                            s = await summarizeFileWithGemini(node.id, code, activeKey);
                        } catch (geminiErr: any) {
                            console.warn(`[Pipeline] Gemini summarization failed for ${node.id}, falling back to local Ollama:`, geminiErr.message);
                ...
                console.error(`[Pipeline] Summarization failed for ${node.id}:`, err.message);
                s = {
        ...
        await store.saveGraph(globalGraph.getAllNodes(), globalGraph.getAllEdges(), durationSoFar);
        ...
        _runningRepos.delete(frontendRepoId);
    }
}
