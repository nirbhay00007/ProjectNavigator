...
import { semanticSearch, initVectorStore, clearVectorStore } from './storage/vectorStore';
...
import * as JavaStore from './storage/javaStore';
...
// ─── GET /api/repos ────────────────────────────────────────────────────
// Returns all previously analyzed repositories for the logged-in user.
// Hydrates the in-memory graph and vector stores with the loaded data.
...
app.get('/api/repos', async (req, res) => {
    ...
    try {
        const repos = await JavaStore.listRepositories(token);
        
        // Reset in-memory stores to prevent accumulation across requests
        globalGraph.clear();
        clearVectorStore();
        ...
        for (const repo of repos) {
            const nodes = await JavaStore.loadFileNodes(repo.id, token);
            const edges = await JavaStore.loadEdges(repo.id, token);
            const vectors = await JavaStore.loadVectors(repo.id, token);
            ...
            // Export layout for React Flow frontend
            const flowData = globalGraph.exportForReactFlow(repo.id);
        ...
        // Re-compute metrics and re-hydrate the global vector index
        if (repos.length > 0) {
            globalGraph.computeMetrics();
        }
        initVectorStore(allVectors);

        res.json({ success: true, repositories: repoEntries });
    } catch (err: any) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error('[API /api/repos] Error:', msg);
...
app.post('/api/analyze', async (req, res) => {
    ...
    try {
        const resolvedRepoId = await runIngestionPipeline(options, res);

        const graphData    = globalGraph.exportForReactFlow(resolvedRepoId);
...
app.post('/api/load', async (req, res) => {
    ...
    try {
        const store = initStore(path.resolve(targetPath), token);
        const persistedGraph = await store.loadGraph();
        const persistedVectors = await store.loadVectors();
...
// ─── DELETE /api/repo ─────────────────────────────────────────────────────────
// Deletes a repository and all its data from the PostgreSQL database.
app.delete('/api/repo', async (req, res) => {
    const token = req.headers.authorization;
    if (!token) {
        return res.status(401).json({ error: 'Unauthorized: Missing Authorization header.' });
    }
    const repoId = (req.body?.repoId || req.query?.repoId) as string;
    if (!repoId) return res.status(400).json({ error: 'repoId is required.' });

    try {
        await JavaStore.deleteRepository(repoId, token);
        res.json({ success: true });
    } catch (err: any) {
        console.error('[/api/repo] Delete error:', err.message);
        res.status(500).json({ error: err.message });
    }
});
